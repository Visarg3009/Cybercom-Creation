...
function age_18() {
    ...
    for (var k = 0; k < array.length; k++) {
        var age = getAge(array[k].birthDate);
        //console.log(age);
        if (age < 18) {
            console.log("into the function")
            count++;
        }
        localStorage.setItem("age_18", count);
        if (age > 50) {
            console.log("into the function")
            count1++;
        }
        localStorage.setItem("age_50", count1);
        if (age > 18 && age < 50) {
            //console.log("into the function")
            count2++;
        }
        localStorage.setItem("age_med", count2);
    ...
    if (document.getElementById("addUser").value == "Update") {
        ...
        for (var h = 0; h < items.length; h++) {
            ...
            if (target1 == items[h].index) {
                items[h].name = document.getElementById('name').value;
